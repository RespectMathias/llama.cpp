...
// DSpark draft model (semi-autoregressive speculative decoding).
//
// The backbone graph is structurally identical to DFlash: an ENCODER that fuses the
// extracted target hidden states (fc + hidden_norm), and a DECODER that cross-attends a
// noise/anchor block against the accumulated target context and emits per-position logits.
//
// What DSpark adds over DFlash -- the Markov head (semi-autoregressive bias) and the
// confidence head (scheduled prefix pruning) -- is NOT part of this graph. Those depend on
// the token sampled at the previous block position, so they are applied CPU-side in the
// speculative loop (see common/speculative.cpp, M3/M4) on top of:
//   - t_logits : base logits at every block position (own lm_head, model.output)
//   - t_embd   : the post-norm hidden state per position (input to the confidence head)
//
// Unlike DFlash, DSpark is NOT tied to the target embeddings: it owns tok_embd / output.

ggml_tensor * llm_build_dspark_encode::build_inp_embd() const {
...
llm_build_dspark_encode::llm_build_dspark_encode(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ggml_tensor * cur = build_inp_embd();

    cur = build_lora_mm(model.fc, cur);
    cb(cur, "fc_out", -1);

    cur = build_norm(cur, model.dspark_hidden_norm, NULL, LLM_NORM_RMS, -1);
...
llm_build_dspark_decode::llm_build_dspark_decode(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    const int64_t n_embd_head = hparams.n_embd_head_v();

    GGML_ASSERT(n_embd_head == hparams.n_embd_head_k());

    // anchor + noise [MASK] tokens, embedded with DSpark's OWN embeddings (not tied to target)
    GGML_ASSERT(model.tok_embd != nullptr && "DSpark decoder requires its own tok_embd");
    ggml_tensor * noise_embd = build_inp_embd(model.tok_embd);
    ...
    // Q positions: last n_tokens entries (noise/anchor only)
    ...
    // post-norm hidden states -- consumed CPU-side by the confidence head (M4)
    res->t_embd = cur;

    // base logits via DSpark's OWN lm_head -- the CPU-side Markov head (M3) adds its bias on top
    GGML_ASSERT(model.output != nullptr && "DSpark decoder requires its own lm_head (output)");
